...
Math.__name__ = true;
var Ogg2WavNodejs = function() { };
Ogg2WavNodejs.__name__ = true;
Ogg2WavNodejs.main = function() {
	var platform = "nodejs";
	...
	while(_g < _g1.length) {
		var file = _g1[_g];
		++_g;
		if(HxOverrides.substr(file,file.length - 4,null) != ".ogg") {
			continue;
		}
		var inPath = "" + dir + "/" + file;
		if(js_node_Fs.statSync(inPath).isDirectory()) {
			continue;
		}
		var outPath = "" + dir + "/wav/" + platform + "-" + HxOverrides.substr(file,0,file.length - 4) + ".wav";
		process.stdout.write(Std.string("File Name : " + inPath));
		process.stdout.write("\n");
		var reader = stb_format_vorbis_Reader.openFromBytes(js_node_buffer__$Buffer_Helper.bytesOfBuffer(js_node_Fs.readFileSync(inPath)));
		...
		process.stdout.write(Std.string("Channel : " + header.channel));
		process.stdout.write("\n");
		process.stdout.write(Std.string("Sample Rate : " + Std.string(_$UInt_UInt_$Impl_$.toFloat(header.sampleRate))));
		process.stdout.write("\n");
		process.stdout.write(Std.string("Sample : " + reader.get_totalSample()));
		process.stdout.write("\n");
		var v = "Time : " + (reader.get_totalMillisecond() / 1000 | 0) + "sec";
		process.stdout.write(Std.string(v));
		process.stdout.write("\n");
		process.stdout.write(Std.string("Vendor : " + header.vendor));
		process.stdout.write("\n");
		...
		while(key.hasNext()) {
			var key1 = key.next();
			var _g2 = 0;
			var _g3 = __map_reserved[key1] != null?commentData.getReserved(key1):commentData.h[key1];
			while(_g2 < _g3.length) {
				var value = _g3[_g2];
				++_g2;
				var v1 = key1.toUpperCase() + "=" + value;
				process.stdout.write(Std.string(v1));
				process.stdout.write("\n");
			}
		}
		var output = new FileOutputY(outPath,reader.get_totalSample() * 8);
		...
		while(true) {
			...
			var v2 = "" + reader.get_currentSample() + "/" + reader.get_totalSample() + " : Decode Time " + Ogg2WavNodejs.lapTime(time);
...
Ogg2WavNodejs.lapTime = function(time) {
...
var FileOutputY = function(path,bufferSize) {
	if(bufferSize == null) {
		bufferSize = 60000000;
	}
	this.o = js_node_Fs.createWriteStream(path);
	this.b = new js_node_buffer_Buffer(bufferSize);
	this.offset = 0;
};
FileOutputY.__name__ = true;
FileOutputY.__super__ = haxe_io_Output;
FileOutputY.prototype = $extend(haxe_io_Output.prototype,{
	writeInt16: function(val) {
		if(this.bigEndian) {
			this.b.writeInt16BE(val,this.offset);
		} else {
			this.b.writeInt16LE(val,this.offset);
		}
		this.offset += 2;
	}
	,writeUInt16: function(val) {
		if(this.bigEndian) {
			this.b.writeUInt16BE(val,this.offset);
		} else {
			this.b.writeUInt16LE(val,this.offset);
		}
		this.offset += 2;
	}
	,writeInt32: function(val) {
		if(this.bigEndian) {
			this.b.writeInt32BE(val,this.offset);
		} else {
			this.b.writeInt32LE(val,this.offset);
		}
		this.offset += 4;
	}
	,writeByte: function(c) {
		var str = String.fromCharCode(c);
		this.b.write(str,this.offset);
		this.offset += str.length;
	}
	,writeBytes: function(s,pos,len) {
		var str = HxOverrides.substr(s.toString(),pos,len);
		this.b.write(str,this.offset);
		this.offset += str.length;
		return str.length;
	}
	,close: function() {
		this.o.write(this.b.slice(0,this.offset));
		this.o.end();
	}
	,__class__: FileOutputY
});
var Std = function() { };
Std.__name__ = true;
...
var FileOutput = function(fd) {
	this.fd = fd;
};
FileOutput.__name__ = true;
FileOutput.__super__ = haxe_io_Output;
FileOutput.prototype = $extend(haxe_io_Output.prototype,{
	writeByte: function(c) {
